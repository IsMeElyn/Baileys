...
const makeMessagesSocket = (config) => {
  ...
  const femboy = new elyn(Utils_1, waUploadToServer, relayMessage);
  ...
  return {
    ...
    sendMessage: async (jid, content, options = {}) => {
      ...
      const messageType = femboy.detectType(content);
      if (
        ...
        if (messageType) {
          switch (messageType) {
            case "PAYMENT":
              const paymentContent = await femboy.handlePayment(
                content,
                quoted,
              );
            ...
            case "PRODUCT":
              const productContent = await femboy.handleProduct(
                content,
                jid,
                quoted,
              );
            ...
            case "INTERACTIVE":
              const interactiveContent = await femboy.handleInteractive(
                content,
                jid,
                quoted,
              );
            ...
            case "ALBUM":
              return await femboy.handleAlbum(content, jid, quoted);
            case "EVENT":
              return await femboy.handleEvent(content, jid, quoted);
            case "POLL_RESULT":
              return await femboy.handlePollResult(content, jid, quoted);
            case "GROUP_STORY":
              return await femboy.handleGroupStory(content, jid, quoted);
          }
        }
