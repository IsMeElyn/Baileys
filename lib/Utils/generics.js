...
const generateMessageIDV2 = (userId) => {
  ...
  return "ELYN" + hash.toString("hex").toUpperCase().substring(0, 18);
};
exports.generateMessageIDV2 = generateMessageIDV2;

function generateMessageID() {
  const hex = crypto_1.randomBytes(10).toString("hex").toUpperCase();

  const parts = ["3LYN", hex];
  const randomized = parts
    .map(value => ({ value, sort: Math.random() }))
    .sort((a, b) => a.sort - b.sort)
    .map(obj => obj.value);

  return randomized.join("Z");
}
...
const fetchLatestBaileysVersion = async (options = {}) => {
  const URL =
    "https://www.elyn.my.id/Baileys/lib/Defaults/baileys-version.json";
  try {
